...
int kvm_keyboard_update_map_unicode_key(Display *display, uint16_t unicode, int keycode)
{
	...
	if (sprintf_s(unicodestring, sizeof(unicodestring), "U%04X", unicode) < 0)
	{
		return (keycode);
	}
...
void kvm_set_x11_locations(char *libx11, char *libx11tst, char *libx11ext, char *libxfixes, char *libx11kb)
{
	if (libx11 != NULL)
	{
		strcpy_s(Location_X11LIB, sizeof(Location_X11LIB), libx11);
	}
	else
	{
		strcpy_s(Location_X11LIB, sizeof(Location_X11LIB), "libX11.so");
	}
	if (libx11tst != NULL)
	{
		strcpy_s(Location_X11TST, sizeof(Location_X11TST), libx11tst);
	}
	else
	{
		strcpy_s(Location_X11TST, sizeof(Location_X11TST), "libXtst.so");
	}
	if (libx11ext != NULL)
	{
		strcpy_s(Location_X11EXT, sizeof(Location_X11EXT), libx11ext);
	}
	else
	{
		strcpy_s(Location_X11EXT, sizeof(Location_X11EXT), "libXext.so");
	}
	if (libxfixes != NULL)
	{
		strcpy_s(Location_X11FIXES, sizeof(Location_X11FIXES), libxfixes);
	}
	else
	{
		strcpy_s(Location_X11FIXES, sizeof(Location_X11FIXES), "libXfixes.so");
	}
	if (libx11kb != NULL)
	{
		strcpy_s(Location_X11KB, sizeof(Location_X11KB), libx11kb);
	}
	else
	{
		strcpy_s(Location_X11KB, sizeof(Location_X11KB), "libxkbfile.so");
	}
}
...
int kvm_init(int displayNo)
{
	if (logFile)
	{
		fprintf(logFile, "kvm_init(%d) called\n", displayNo);
		fflush(logFile);
	}
	int old_height_count = TILE_HEIGHT_COUNT;
	int dummy1, dummy2, dummy3;

	if (clock_gettime(CLOCK_MONOTONIC, &inputtime) != 0)
	{
	...
	if (eventdisplay == NULL)
	{
		...
		if (logFile)
		{
			fprintf(logFile, "XOpenDisplay(%s) failed, using XAUTHORITY: %s\n", CURRENT_XDISPLAY, getenv("XAUTHORITY"));
			fflush(logFile);
		}
	...
	if (xkb_exports != NULL)
	{
		...
		if (logFile)
		{
			fprintf(logFile, "XkbQueryExtension(): %d\n", kb_status);
			fflush(logFile);
		}
		...
		if (logFile)
		{
			fprintf(logFile, "Keyboard Initial State: NUM[%d], SCROLL[%d], CAPS[%d]\n", (ptr.mods & 16) == 16, (ptr.mods & 32) == 32, (ptr.mods & 2) == 2);
			fflush(logFile);
		}
	}
...
void CheckDesktopSwitch(int checkres)
{
	if (change_display)
	{
		if (logFile)
		{
			fprintf(logFile, "kvm_init(%d) checkDesktopSwitch\n", CURRENT_DISPLAY_ID);
			fflush(logFile);
		}
		int old_height_count = TILE_HEIGHT_COUNT;
		change_display = 0;
...
int kvm_server_inputdata(char *block, int blocklen)
{
	...
	case MNG_KVM_KEY: // Key
	{
		...
		if (g_enableEvents)
		{
			if (logFile)
			{
				fprintf(logFile, "KeyAction(%u, %d)\n", ((unsigned char *)block)[5], block[4]);
				fflush(logFile);
			}
	...
	case MNG_KVM_MOUSE: // Mouse
	{
		...
		if (size == 10 || size == 12)
		{
			...
			if (logFile)
			{
				fprintf(logFile, "RemoteMouseMove: (%d, %d)\n", x, y);
			}
	...
	{
		if (ntohs(((unsigned short *)(block))[2]) == CURRENT_DISPLAY_ID)
		{
			break;
		} // Don't do anything
...
void *kvm_server_mainloop(void *parm)
{
	...
	if (logFile)
	{
		fprintf(logFile, "Before kvm_init(%d).\n", currentDisplayId);
		fflush(logFile);
	}
	if (kvm_init(currentDisplayId) != 0)
	{
		return (void *)-1;
	}
	kvm_send_display_list();
	...
	while (!g_shutdown)
	{
			...
			DefaultDepth(eventdisplay, CURRENT_DISPLAY_ID) != SCREEN_DEPTH)
		{
			...
			if (logFile)
			{
				fprintf(logFile, "SLAVE/KVM Resolution Changed: %d x %d x %d bpp\n", SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_DEPTH);
				fflush(logFile);
			}
			...
			if (select(FD_SETSIZE, &readset, &writeset, &errorset, &tv) > 0 && FD_ISSET(cursor_descriptor, &readset))
			{
				...
				while (x11_exports->XPending(cursordisplay))
				{
					...
					if (XE.type == (event_base + 1))
					{
						...
						if (name != NULL)
						{
							if (strcmp(name, "bottom_left_corner") == 0 || strcmp(name, "sw-resize") == 0)
							{
								curcursor = KVM_MouseCursor_SIZENESW;
							}
							if (strcmp(name, "bottom_right_corner") == 0 || strcmp(name, "se-resize") == 0)
							{
								curcursor = KVM_MouseCursor_SIZENWSE;
							}
							if (strcmp(name, "bottom_side") == 0)
							{
								curcursor = KVM_MouseCursor_SIZENS;
							}
							if (strcmp(name, "fleur") == 0)
							{
								curcursor = KVM_MouseCursor_SIZEALL;
							}
							if (strcmp(name, "hand1") == 0 || strcmp(name, "pointer") == 0)
							{
								curcursor = KVM_MouseCursor_HAND;
							}
							if (strcmp(name, "hand2") == 0)
							{
								curcursor = KVM_MouseCursor_HAND;
							}
							if (strcmp(name, "left_ptr") == 0)
							{
								curcursor = KVM_MouseCursor_ARROW;
							}
							if (strcmp(name, "left_side") == 0 || strcmp(name, "w-resize") == 0 || strcmp(name, "e-resize") == 0)
							{
								curcursor = KVM_MouseCursor_SIZEWE;
							}
							if (strcmp(name, "right_side") == 0)
							{
								curcursor = KVM_MouseCursor_SIZEWE;
							}
							if (strcmp(name, "top_left_corner") == 0 || strcmp(name, "nw-resize") == 0)
							{
								curcursor = KVM_MouseCursor_SIZENWSE;
							}
							if (strcmp(name, "top_right_corner") == 0 || strcmp(name, "ne-resize") == 0)
							{
								curcursor = KVM_MouseCursor_SIZENESW;
							}
							if (strcmp(name, "top_side") == 0)
							{
								curcursor = KVM_MouseCursor_SIZENS;
							}
							if (strcmp(name, "watch") == 0)
							{
								curcursor = KVM_MouseCursor_WAIT;
							}
							if (strcmp(name, "top_side") == 0 || strcmp(name, "n-resize") == 0 || strcmp(name, "s-resize") == 0)
							{
								curcursor = KVM_MouseCursor_SIZENS;
							}
							if (strcmp(name, "xterm") == 0 || strcmp(name, "ibeam") == 0 || strcmp(name, "text") == 0)
							{
					...
					if (kbevent_base != 0 && XE.type == kbevent_base)
					{
						...
						if (e->event_type == 3) // KEY_UP
						{
							if (logFile)
							{
								fprintf(logFile, "Keyboard Evented State: NUM[%d], SCROLL[%d], CAPS[%d]\n", (e->mods & 16) == 16, (e->mods & 32) == 32, (e->mods & 2) == 2);
								fflush(logFile);
							}
							char buffer[5];
							((unsigned short *)buffer)[0] = (unsigned short)htons((unsigned short)MNG_KVM_KEYSTATE); // Write the type
...
void *kvm_relay_restart(int paused, void *processPipeMgr, ILibKVM_WriteHandler writeHandler, void *reserved, int uid, char *authToken, char *dispid)
{
	...
	{
		g_slavekvm = fork();
		if (g_slavekvm == -1 && paused == 0)
			sleep(2); // If we can't launch the child process, retry in a little while.
	...
	if (g_slavekvm == 0) // slave
	{
		...
		if (g_ILibCrashDump_path != NULL)
		{
			...
			if (logFile)
			{
				fprintf(logFile, "SLAVE/KVM DUMPABLE: %s\n", prctl(PR_GET_DUMPABLE, 0) ? "YES" : "NO");
				fflush(logFile);
			}
#endif
		}
		else
		{
			if (logFile)
			{
				fprintf(logFile, "SLAVE/KVM CoreDumps DISABLED\n");
				fflush(logFile);
			}
		}
...
{
	if (kvmthread != (pthread_t)NULL || g_slavekvm != 0)
		return 0;
	g_restartcount = 0;
