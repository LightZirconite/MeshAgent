...
int adjust_screen_size(int pixles)
{
	...
	if (extra != 0)
		return pixles + TILE_WIDTH - extra;
	return pixles;
}
...
int calc_opt_compr_send(int x, int y, int captureWidth, int captureHeight, void *desktop, void **buffer, long long *bufferSize)
{
	...
	*bufferSize = 0;
	...
	if (tilebuffersize != bmpInfo.bmiHeader.biSizeImage)
	{
		...
		if ((tilebuffer = malloc(tilebuffersize)) == NULL)
			return 0;
	}
...
extern "C"
{
	...
	int get_tile_at(int x, int y, void **buffer, long long *bufferSize, void *desktop, int row, int col)
	{
		...
		*bufferSize = 0;
		...
		if (tileInfo[row][col].flags == (char)TILE_TODO) // First check whether the tile-crc needs to be calcualted or not.
		{
			...
			if ((CRC = tile_crc(x, y, desktop, TILE_WIDTH, TILE_HEIGHT)) == tileInfo[row][col].crc)
				return 0;
	...
	int get_desktop_buffer(void **buffer, long long *bufferSize, long *mouseMove)
	{
		...
		*bufferSize = 0;
		...
		if ((hDesktopDC = GetDC(NULL)) == NULL)
		{
			KVMDEBUG("GetDC(NULL) returned NULL", 0);
			return 1;
		} // We need to do this incase the current desktop changes.
		if (hCapturedBitmap)
		...
		if (SelectObject(hCaptureDC, hCapturedBitmap) == NULL)
		{
		...
		else
		{
			...
			{
				KVMDEBUG("SetStretchBltMode() failed", 0);
				return (1);
			}
	...
	short initialize_gdiplus()
	{
		...
		if ((hDesktopDC = GetDC(NULL)) == NULL)
		{
			KVMDEBUG("GetDC() failed", 0);
			return 0;
		}
		if ((hCaptureDC = CreateCompatibleDC(hDesktopDC)) == NULL)
		{
			KVMDEBUG("CreateCompatibleDC() failed", 0);
			return 0;
		}
		if ((hCapturedBitmap = CreateCompatibleBitmap(hDesktopDC, SCALED_WIDTH, SCALED_HEIGHT)) == NULL)
		{
			KVMDEBUG("CreateCompatibleBitmap() failed", 0);
			return 0;
		}
		if (SelectObject(hCaptureDC, hCapturedBitmap) == NULL)
		{
	...
	void set_tile_compression(int type, int level)
	{
		...
		if (encCompression > 90)
		{
			encCompression = 90;
		}
		if (tilebuffer == NULL)
		{
			KVMDEBUG("set_tile_compression(), tilebuffer == NULL.", 0);
			return;
		}
