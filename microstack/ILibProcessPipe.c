...
ILibProcessPipe_Process ILibProcessPipe_Manager_SpawnProcessEx4(ILibProcessPipe_Manager pipeManager, char *target, char *const *parameters, ILibProcessPipe_SpawnTypes spawnType, void *sid, void *envvars, int extraMemorySize)
{
	...
	if (spawnType != ILibProcessPipe_SpawnTypes_DEFAULT && spawnType != ILibProcessPipe_SpawnTypes_DETACHED)
	{
		...
		else if (spawnType == ILibProcessPipe_SpawnTypes_USER || spawnType == ILibProcessPipe_SpawnTypes_SPECIFIED_USER)
		{
			// Without this, the child process inherits the service's non-interactive window station
			// (Service-0x0-3e7$). DXGI DuplicateOutput then binds to the wrong DWM compositor context
			// and returns black frames even though AcquireNextFrame returns S_OK with LastPresentTime != 0.
			// LocalSystem already has GENERIC_ALL on WinSta0\Default so no DACL change is needed.
