...
char *ILibDuktape_fs_fixLinuxPath(char *path)
{
	...
	int len = end;
	if (end > (sizeof(ILibDuktape_fs_linuxPath) - 1))
	{
		return (NULL);
	}
...
duk_ret_t ILibDuktape_fs_openSync(duk_context *ctx)
{
	...
	if (duk_is_string(ctx, 1))
	{
		...
		if (nargs < 2)
		{
			return (ILibDuktape_Error(ctx, "Too few arguments"));
		}
	...
	if (((flags & _O_RDONLY) == _O_RDONLY) || ((flags & _O_RDWR) == _O_RDWR))
	{
		dwDesiredAccess |= GENERIC_READ;
	}
	if (((flags & _O_WRONLY) == _O_WRONLY) || ((flags & _O_RDWR) == _O_RDWR))
	{
...
duk_ret_t ILibDuktape_fs_readSync(duk_context *ctx)
{
	...
	if (narg > 3)
	{
		...
		duk_dup(ctx, 0);
		duk_dup(ctx, 1);	  // [fs][readSync][this][fd][buffer]
	...
	{
		return (ILibDuktape_Error(ctx, "fs.readSync(): Buffer of size: %llu bytes, but attempting to read %d bytes", (uint64_t)bufferSize, length));
	}
	if (f != NULL)
	{
		if (position >= 0)
...
duk_ret_t ILibDuktape_fs_read_readsetSink(duk_context *ctx)
{
	...
	if (duk_get_length(ctx, -1) == 0)
	{
		...
		duk_push_true(ctx);
		duk_put_prop_string(ctx, -2, "readset"); //..[removeDescriptor][this][fd][options]
		duk_pcall_method(ctx, 2);
		duk_pop(ctx); // [DescriptorEvents][pending]
...
duk_ret_t ILibDuktape_fs_write_writeset_sink(duk_context *ctx)
{
	...
	if (performCleanup != 0)
	{
		...
		duk_push_true(ctx);
		duk_put_prop_string(ctx, -2, "writeset"); //[remove][this][fd][options]
...
BOOL ILibDuktape_fs_write_WindowsSink(void *chain, HANDLE h, ILibWaitHandle_ErrorStatus status, DWORD bytesWritten, void *user)
{
	...
	if (ILibMemory_CanaryOK(data) && duk_ctx_is_alive(data->ctx))
	{
		...
		if (duk_pcall_method(data->ctx, 3) != 0)
		{
			ILibDuktape_Process_UncaughtExceptionEx(data->ctx, "fs.write.onCallack(): ");
		}
		duk_pop(ctx); // ...
	}
	return (FALSE);
...
duk_ret_t ILibDuktape_fs_write(duk_context *ctx)
{
	...
	{
		offset = (int)duk_require_int(ctx, 2);
		cbx++;
	}
	if (duk_is_number(ctx, 3))
	{
		length = (int)duk_require_int(ctx, 3);
		cbx++;
	}
	...
	if (position >= 0)
	{
		...
		if (loworder == INVALID_SET_FILE_POINTER && GetLastError() != NO_ERROR)
		{
			return (ILibDuktape_Error(ctx, "Unable to seek to Position"));
		}
	...
	if (bytesWritten > 0 || (e = errno) == EAGAIN || e == EWOULDBLOCK || e == EINTR)
	{
		...
		if (!duk_has_prop_index(ctx, -1, fd))
		{
			...
			duk_push_true(ctx);
			duk_put_prop_string(ctx, -2, "writeset"); // ...[add][this][fd][options]
			if (duk_is_object(ctx, cbx + 1) && duk_has_prop_string(ctx, cbx + 1, "metadata"))
			{
				...
				duk_string_concat(ctx, -2);
				duk_remove(ctx, -2); // [fs][table][add][this][fd][options][metadata]
			}
			else
			{
		...
		duk_dup(ctx, cbx);
		duk_put_prop_string(ctx, -2, "callback");			   // [fs][table][events][pending]
...
BOOL ILibDuktape_fs_read_WindowsSink(void *chain, HANDLE h, ILibWaitHandle_ErrorStatus status, char *buffer, DWORD bytesRead, void *user)
{
	...
	if (ILibMemory_CanaryOK(data) && duk_ctx_is_alive(data->ctx))
	{
		...
		if (duk_pcall_method(data->ctx, 3) != 0)
		{
			ILibDuktape_Process_UncaughtExceptionEx(ctx, "fs.read.onCallack(): ");
		}
		duk_pop(ctx); // ...
	}
	return (FALSE);
...
duk_ret_t ILibDuktape_fs_read(duk_context *ctx)
{
	...
	{
		if (strcmp(Duktape_GetStringPropertyValue(ctx, 0, "_ObjectID", ""), "fs.bufferDescriptor") != 0)
		{
		...
		if (bytesRead > ((int)wrbufLen - offset))
		{
			bytesRead = (int)wrbufLen - offset;
		}
		duk_size_t srbufLen;
	...
	if (!(duk_is_number(ctx, 0) && duk_is_object(ctx, 1) && duk_is_function(ctx, 2)))
	{
		return (ILibDuktape_Error(ctx, "Invalid Parameters"));
	}
#ifdef WIN32
	...
	if (!duk_has_prop_index(ctx, -1, fd))
	{
		...
		duk_push_true(ctx);
		duk_put_prop_string(ctx, -2, "readset");
		if (duk_has_prop_string(ctx, 1, "metadata"))
		{
			...
			duk_string_concat(ctx, -2);
			duk_remove(ctx, -2);					  // [fs][table][addDescriptor][this][fd][options][metadata]
			duk_put_prop_string(ctx, -2, "metadata"); // [fs][table][addDescriptor][this][fd][options]
		}
		else
		{
			duk_push_string(ctx, "fs.read()");
		...
		duk_call_method(ctx, 2);
		duk_pop(ctx); // [fs][table][descriptorEvent]
		duk_push_array(ctx);
		duk_put_prop_string(ctx, -2, FS_EVENT_DESCRIPTORS_IO);
		duk_put_prop_index(ctx, -2, fd); // [fs][table]
	}
	...
	duk_dup(ctx, 2);
	duk_put_prop_string(ctx, -2, "callback"); // [fs][table][desriptorEvent][pending][options]
...
duk_ret_t ILibDuktape_fs_writeSync(duk_context *ctx)
{
	...
	if (nargs > 2)
	{
		buffer = buffer + duk_require_int(ctx, 2);
	}
	if (nargs > 3)
	{
		length = (duk_size_t)duk_require_int(ctx, 3);
	}
...
void ILibDuktape_fs_readStream_Resume(struct ILibDuktape_readableStream *sender, void *user)
{
	...
	while (sender->paused == 0 && data->bytesRead > 0 && (data->bytesLeft < 0 || data->bytesLeft > 0))
	{
		...
		if (data->bytesRead > 0)
		{
			if (data->bytesLeft > 0)
			{
				data->bytesLeft -= data->bytesRead;
			}
			data->bytesRead += data->unshiftedBytes;
			data->unshiftedBytes = 0;
			do
			{
				...
				if (data->unshiftedBytes > 0 && data->unshiftedBytes != preshift)
				{
					memmove(data->buffer, data->buffer + (preshift - data->unshiftedBytes), data->unshiftedBytes);
				}
...
duk_ret_t ILibDuktape_fs_statSync(duk_context *ctx)
{
	...
	{
		return (ILibDuktape_Error(ctx, "fs.statSync(): Path Error [%s]", path));
	}
	...
	duk_push_int(ctx, (int)result.st_uid);
	duk_put_prop_string(ctx, -2, "uid");
	duk_push_int(ctx, (int)result.st_gid);
...
duk_ret_t ILibDuktape_fs_readDrivesSync(duk_context *ctx)
{
	...
	if (ILibDuktape_fs_readDrivesSync_result_PUSH(ctx, volumeName) != 0)
	{
		duk_put_prop_index(ctx, -2, i++);
	}
...
duk_ret_t ILibDuktape_fs_watcher_close(duk_context *ctx)
{
	...
	if (!duk_has_prop_string(ctx, -1, FS_WATCHER_DATA_PTR))
	{
		return (0);
	}
...
{
	if (errors != ILibWaitHandle_ErrorStatus_NONE || !ILibMemory_CanaryOK(user))
	{
		return (FALSE);
	}
	...
	if (duk_pcall_method(data->ctx, 4) != 0)
	{
		ILibDuktape_Process_UncaughtException(data->ctx);
	}
	duk_pop_2(data->ctx); // ...
...
char *ILibDuktape_fs_notifyDispatcher_Query(void *chain, void *object, int fd, size_t *dataLen)
{
	...
	if (data->fd != fd)
	{
		return (((ILibChain_Link *)object)->MetaData);
	}
...
void ILibDuktape_fs_notifyDispatcher_PostSelect(void *object, int slct, fd_set *readset, fd_set *writeset, fd_set *errorset)
{
	...
	while ((len = read(data->fd, buffer, sizeof(buffer))) > 0)
	{
		while (i < len)
		{
			...
			if (watcher == NULL || ILibDuktape_EventEmitter_HasListeners(watcher->emitter, "change") == 0)
			{
				continue;
			} // If nobody is listening for this event, no need to waste cycles processing it
			...
			if (duk_pcall_method(watcher->ctx, 4) != 0)
			{
				ILibDuktape_Process_UncaughtException(watcher->ctx);
			}
...
void ILibduktape_fs_watch_appleWorker_MODIFIED(void *chain, void *user, char *changeType)
{
	...
	if (ILibMemory_CanaryOK(data))
	{
		...
		duk_push_string(data->ctx, changeType);
		duk_put_prop_string(data->ctx, -2, "changeType");
		if (duk_pcall_method(data->ctx, 4) != 0)
		{
			ILibDuktape_Process_UncaughtExceptionEx(data->ctx, "fs.fsWatch.onChange(): ");
		}
		duk_pop(data->ctx); // ...
	}
}
	...
	if (ILibMemory_CanaryOK(data))
	{
		if (duk_peval_string(data->ctx, "require('fs');") == 0)
		{
			...
			if (duk_pcall_method(data->ctx, 1) == 0)
			{
				...
				duk_push_string(data->ctx, "rename");
				duk_put_prop_string(data->ctx, -2, "changeType");
				duk_dup(data->ctx, -7);
				duk_put_prop_string(data->ctx, -2, "newname"); // [NAME][emit][this][change][eventType][oldName][detail]
				duk_get_prop_string(data->ctx, -5, "\xFF_FileName");
				duk_put_prop_string(data->ctx, -2, "oldname");
...
void ILibduktape_fs_watch_appleWorker(void *obj)
{
	...
	{
		n = kevent(watcher->kq, change, inCount, &event, 1, NULL);
		inCount = 0;
		if (n > 0)
		{
...
duk_ret_t ILibDuktape_fs_watch(duk_context *ctx)
{
	...
	duk_push_this(ctx);
	duk_put_prop_string(ctx, -2, FS_WATCHER_2_FS);
	duk_dup(ctx, 0);
	duk_put_prop_string(ctx, -2, FS_WATCH_PATH);
	data->emitter = ILibDuktape_EventEmitter_Create(ctx);
	data->ctx = ctx;
...
#if defined(WIN32)
	if ((data->overlapped.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL)) == NULL)
	{
		return (ILibDuktape_Error(ctx, "Could not create handle"));
	}
	...
	if (data->h == INVALID_HANDLE_VALUE)
	{
		return (ILibDuktape_Error(ctx, "fs.watch(): Invalid Path or Access Denied"));
	}
...
duk_ret_t ILibDuktape_fs_unlink(duk_context *ctx)
{
	...
	{
#ifdef WIN32
		if (RemoveDirectoryW((LPCWSTR)path) != 0)
		{
			return 0;
		}
#endif
...
{
	if (!(duk_is_object(ctx, 0) && duk_has_prop_string(ctx, -1, "_ptr")))
	{
		return (ILibDuktape_Error(ctx, "Invalid Input Parameters"));
	}
...
void ILibDuktape_fs_PUSH(duk_context *ctx, void *chain)
{
	...
	duk_push_object(ctx);
#ifdef WIN32
	duk_push_int(ctx, _O_RDONLY);
	duk_put_prop_string(ctx, -2, "O_RDONLY");
	duk_push_int(ctx, _O_WRONLY);
	duk_put_prop_string(ctx, -2, "O_WRONLY");
	duk_push_int(ctx, _O_RDWR);
	duk_put_prop_string(ctx, -2, "O_RDWR");
	duk_push_int(ctx, WIN_FAKE_O_NONBLOCK);
	duk_put_prop_string(ctx, -2, "O_NONBLOCK");
#else
