...
function mshLength() {
  ...
  if (
    buffer.slice(4).toString("hex") == exeMeshPolicyGuid ||
    buffer.slice(4).toString("hex") == exeNullPolicyGuid
  ) {
...
function addMsh(options) {
  ...
  if (
    (options.platform == "win32" &&
      options.peinfo.CertificateTableAddress == 0) ||
    options.platform != "win32"
  ) {
    ...
    options.destinationStream.sourceStream.on("end", function () {
      ...
      if (this.options.randomGuid) {
        mshBuf.randomFill();
      }
      this.options.destinationStream.write(mshBuf, function () {
        this.end();
      }); // GUID
    });
    // Pipe the entire source binary without ending the stream.
    options.destinationStream.sourceStream.pipe(options.destinationStream, {
      end: false,
    });
  } else if (
    options.platform == "win32" &&
    ...
    options.mshPadding =
      (8 -
        ((options.peinfo.certificateDwLength + options.msh.length + 20) % 8)) %
      8; // Compute the padding with quad-align

    console.log("old table size = " + options.peinfo.CertificateTableSize);
    options.peinfo.CertificateTableSize +=
      options.msh.length + 20 + options.mshPadding; // Add to the certificate table size
    console.log("new table size = " + options.peinfo.CertificateTableSize);
    console.log(
      "old certificate dwLength = " + options.peinfo.certificateDwLength,
    );
    options.peinfo.certificateDwLength +=
      options.msh.length + 20 + options.mshPadding; // Add to the certificate size
    console.log(
      "new certificate dwLength = " + options.peinfo.certificateDwLength,
    );
      ...
      source2.on("end", function () {
        ...
        source3.on("end", function () {
          ...
          if (this.options.mshPadding > 0) {
            this.options.destinationStream.write(
              Buffer.alloc(this.options.mshPadding),
            ); // Padding
          }
    ...
    this.options.destinationStream.sourceStream.pipe(
      this.options.destinationStream,
      { end: false },
    );
  }
}
...
try {
  ...
  if (process.argv.length != 5 || outputFile == null || inputFile == null) {
    console.log(
      "usage: " +
        process.execPath.split(process.platform == "win32" ? "\\" : "/").pop() +
  ...
  var options = {
    destinationStream: require("fs").createWriteStream(outputFile, {
      flags: "wb",
    }),
  ...
  options.destinationStream.on("close", function () {
    console.log("DONE");
    process.exit();
  });
  addMsh(options);
