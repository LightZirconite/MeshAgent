...
function prepareFolders(folderPath) {
  ...
  while (tokens.length > 0) {
    ...
    if (path.indexOf(process.platform == "win32" ? "\\" : "/") < 0) {
      continue;
    }
...
function parseServiceStatus(token) {
  ...
  if ((controlsAccepted & 0x00000008) == 0x00000008) {
    j.controlsAccepted.push("SERVICE_CONTROL_PARAMCHANGE");
  }
  if ((controlsAccepted & 0x00000002) == 0x00000002) {
    j.controlsAccepted.push("SERVICE_CONTROL_PAUSE");
    j.controlsAccepted.push("SERVICE_CONTROL_CONTINUE");
  }
  if ((controlsAccepted & 0x00000100) == 0x00000100) {
    j.controlsAccepted.push("SERVICE_CONTROL_PRESHUTDOWN");
  }
  if ((controlsAccepted & 0x00000004) == 0x00000004) {
    j.controlsAccepted.push("SERVICE_CONTROL_SHUTDOWN");
  }
  if ((controlsAccepted & 0x00000001) == 0x00000001) {
    j.controlsAccepted.push("SERVICE_CONTROL_STOP");
  }
  if ((controlsAccepted & 0x00000020) == 0x00000020) {
    j.controlsAccepted.push("SERVICE_CONTROL_HARDWAREPROFILECHANGE");
  }
  if ((controlsAccepted & 0x00000040) == 0x00000040) {
    j.controlsAccepted.push("SERVICE_CONTROL_POWEREVENT");
  }
  if ((controlsAccepted & 0x00000080) == 0x00000080) {
...
if (process.platform == "linux") {
  function _upstart_GetServiceTable() {
    ...
    child.stdin.write(
      "      sname=substr(name[1],0,length(name[1])-length(state[c2])-1); ",
    );
    ...
    child.stdin.write("      } ");
    child.stdin.write(
      '      printf "%s\\"%s\\": {\\"state\\": \\"%s\\", \\"pid\\":\\"%s\\"}",(i==1?"":","),sname,rs,spid; ',
    );
    child.stdin.write("   } ");
    child.stdin.write('   printf "}"; ');
    child.stdin.write("}'\nexit\n");
  ...
  function _systemd_GetServiceTable() {
    ...
    child.stdin.write(
      "systemctl --type=service --state=running | grep .service | tr '\n' '`' | awk -F'`' '",
    );
    child.stdin.write("{");
    child.stdin.write('   printf "{"; ');
    child.stdin.write("   for(i=1;i<NF;++i) ");
    ...
    child.stdin.write('      c=split($i,name," "); ');
    child.stdin.write(
      '      printf "%s\\"%s\\": \\"%s\\"", (i==1?"":","), name[1], name[4]; ',
    );
    child.stdin.write("   } ");
    child.stdin.write('   printf "}"; ');
    child.stdin.write("}'\nexit\n");
...
if (process.platform == "darwin") {
  function getOSVersion() {
    ...
    var ret = {
      raw: child.stdout.str.trim().split("."),
      toString: function () {
        return this.raw.join(".");
      },
    };
    ret.compareTo = function compareTo(val) {
      var raw = typeof val == "string" ? val.split(".") : val.raw;
      if (!raw) {
      ...
      while (self.length > 0 && raw.length > 0) {
        s = parseInt(self.shift());
        r = parseInt(raw.shift());
        if (s < r) {
          return -1;
        }
        if (s > r) {
  ...
  function fetchPlist(folder, name, userid) {
    ...
    if (!require("fs").existsSync(folder + "/" + name + ".plist")) {
      ...
      for (var file in files) {
        ...
        if (child.stdout.str.trim() == name) {
          ret.name = files[file].endsWith(".plist")
            ? files[file].substring(0, files[file].length - 6)
            : files[file];
          Object.defineProperty(ret, "alias", { value: name });
          Object.defineProperty(ret, "plist", {
            value: folder + "/" + files[file],
          });
          break;
        }
      }
      if (ret.name == name) {
        throw (
          " " +
          (folder.split("LaunchDaemon").length > 1
            ? "LaunchDaemon"
            : "LaunchAgent") +
          " (" +
          name +
    ...
    Object.defineProperty(ret, "daemon", {
      value: ret.plist.split("/LaunchDaemons/").length > 1 ? true : false,
    });
    try {
      Object.defineProperty(ret, "installedDate", {
        value: require("fs").statSync(ret.plist).ctime,
      });
    } catch (xx) {}
    ...
    Object.defineProperty(ret, "_keepAlive", {
      get: function () {
        ...
        child.stdin.write(
          ' else { split(c[1], ka, "/"); if(ka[1]=="true") {print "ALWAYS";} } }\'\nexit\n',
        );
        child.waitExit();
        return child.stdout.str.trim();
      },
    ...
    ret.getPID = function getPID(uid, asString) {
      ...
      if (getOSVersion().compareTo("10.10") < 0) {
        ...
        if (uid == null) {
          command =
            'launchctl print system | grep "' +
            this.alias +
            '" | awk \'{ if(split($0, tmp, " ")==3) { if($3=="' +
            this.alias +
            "\") { print $1; } }}'\nexit\n";
        } else {
          command =
    ...
    ret.unload = function unload(uid) {
      ...
      if (useBootout) {
        if (uid == null) {
          child.stdin.write(
            "launchctl bootout system " + this.plist + "\nexit\n",
          );
        } else {
          child.stdin.write(
            "launchctl bootout gui/" + uid + " " + this.plist + "\nexit\n",
          );
        }
      } else {
    ...
    ret.start = function start(uid) {
      ...
      if (!this.daemon && uid == null) {
        uid = self;
      }
      if (!this.daemon && uid > 0 && self == 0) {
        options = { uid: uid };
      }
      if (!this.daemon && uid > 0 && self != 0 && uid != self) {
        throw "Cannot start LaunchAgent into another user domain while not root";
      }
      if (this.daemon && self != 0) {
        throw "Cannot start LaunchDaemon while not root";
      }
    ...
    ret.stop = function stop(uid) {
      ...
      if (!this.daemon && uid > 0 && self != 0 && uid != self) {
        throw "Cannot stop LaunchAgent in another user domain while not root";
      }
      if (this.daemon && self != 0) {
        throw "Cannot stop LaunchDaemon while not root";
      }
    ...
    ret.restart = function restart(uid) {
      ...
      if (getOSVersion().compareTo("10.10") < 0) {
        if (!this.daemon && uid == null) {
          uid = require("user-sessions").Self();
        }
        var command =
          "launchctl unload " +
          this.plist +
        ...
        var command = this.daemon
          ? "system/" + this.alias
          : "gui/" +
            (uid != null ? uid : require("user-sessions").Self()) +
            "/" +
            this.alias;
        var child = require("child_process").execFile("/bin/sh", ["sh"]);
        child.stdout.str = "";
...
function serviceManager() {
  ...
  if (process.platform == "win32") {
    ...
    this.isAdmin = function isAdmin() {
      ...
      if (
        this.proxy.AllocateAndInitializeSid(
          NTAuthority,
          2,
          32,
          544,
        ...
        if (
          this.proxy.CheckTokenMembership(
            0,
            AdministratorsGroup.Deref(),
            member,
          ).Val != 0
        ) {
          if (member.toBuffer().readUInt32LE() != 0) {
            admin = true;
          }
        }
    ...
    this.enumerateService = function () {
      ...
      var success = this.proxy.EnumServicesStatusExW(
        handle,
        0,
        0x00000030,
        0x00000003,
      ...
      this.proxy.EnumServicesStatusExW(
        handle,
        0,
        0x00000030,
        0x00000003,
      ...
      for (
        var i = 0;
        i < servicesReturned.Deref(0, dbName._size).toBuffer().readUInt32LE();
        ++i
      ) {
    ...
    this.getService = function getService(name) {
      ...
      var handle = this.proxy.OpenSCManagerA(
        0x00,
        0x00,
        0x0001 | 0x0004 | 0x0010 | (isroot ? 0x0020 : 0x00),
      );
      if (handle.Val == 0) {
        throw "could not open ServiceManager";
      }
      var h = this.proxy.OpenServiceW(
        handle,
        serviceName,
        0x0001 |
          0x0004 |
          (isroot ? 0x0002 | 0x0020 | 0x0010 | 0x00010000 : 0x00),
      );
      if (h.Val != 0) {
        ...
        Object.defineProperty(retVal, "status", {
          get: function () {
            ...
            this._proxy.QueryServiceStatusEx(
              this._service,
              0,
              0,
              0,
            ...
            var st = this._GM.CreateVariable(
              bytesNeeded.toBuffer().readUInt32LE(),
            );
            if (
              this._proxy.QueryServiceStatusEx(
                this._service,
          ...
          get: function () {
            ...
            try {
              return reg.QueryKey(
                ...
                "SYSTEM\\CurrentControlSet\\Services\\" + this.name,
                "_InstalledBy",
              );
            } catch (xx) {
              return null;
        ...
        try {
          Object.defineProperty(retVal, "installedDate", {
            value: require("win-registry").QueryKeyLastModified(
        ...
        if (retVal.status.state != "UNKNOWN") {
          ...
          retVal.update = function update() {
            if (this.failureActions) {
              var actions = this._GM.CreateVariable(
                this.failureActions.actions.length * 8,
              ); // len*sizeof(SC_ACTION)
              for (
                var i = 0;
                i < this.failureActions.actions.length && i < 3;
                ++i
              ) {
                actions
                  .Deref(i * 8, 4)
                  .toBuffer()
                  .writeUInt32LE(
                    failureActionToInteger(this.failureActions.actions[i].type),
                  ); // SC_ACTION[i].type
                actions
                  .Deref(4 + i * 8, 4)
                  .toBuffer()
                  .writeUInt32LE(this.failureActions.actions[i].delay); // SC_ACTION[i].delay
              }

              var updatedFailureActions = this._GM.CreateVariable(40); // sizeof(SERVICE_FAILURE_ACTIONS)
              updatedFailureActions
                .Deref(0, 4)
                .toBuffer()
                .writeUInt32LE(this.failureActions.resetPeriod); // dwResetPeriod
              updatedFailureActions
                .Deref(this._GM.PointerSize == 8 ? 24 : 12, 4)
                .toBuffer()
                .writeUInt32LE(this.failureActions.actions.length); // cActions
              actions
                .pointerBuffer()
                .copy(
                  updatedFailureActions
                    .Deref(
                      this._GM.PointerSize == 8 ? 32 : 16,
                      this._GM.PointerSize,
                    )
                    .toBuffer(),
                );
              if (
                this._proxy.ChangeServiceConfig2W(
                  this._service,
                  2,
                  updatedFailureActions,
                ).Val == 0
          ...
          retVal.stop = function () {
            ...
            if (status.state == "RUNNING") {
              ...
              var reason;
              if (
                this._proxy.ControlService(this._service, 0x00000001, newstate)
                  .Val == 0 &&
                (reason = this._proxy2.GetLastError().Val) != 0
              ) {
                ...
                if (ret._waitTime < 500) {
                  ret._waitTime = 500;
                }
                if (ret._waitTime > 5000) {
                  ret._waitTime = 5000;
                }
          ...
          retVal.restart = function () {
            if (this.isMe()) {
              ...
              var p = this.stop();
              p.startp = new promise(function (a, r) {
                this._a = a;
                this._r = r;
              });
              p.service = this;
          ...
          if (query_service_configa_DWORD.toBuffer().readUInt32LE() > 0) {
            var query_service_configa = this.GM.CreateVariable(
              query_service_configa_DWORD.toBuffer().readUInt32LE(),
            );
            if (
              this.proxy.QueryServiceConfigA(
                h,
                query_service_configa,
                query_service_configa._size,
                query_service_configa_DWORD,
              ).Val != 0
            ) {
              var val = query_service_configa
                .Deref(this.GM.PointerSize == 4 ? 28 : 48, this.GM.PointerSize)
                .Deref().String;
              Object.defineProperty(retVal, "user", { value: val });
              switch (
                query_service_configa.Deref(4, 4).toBuffer().readUInt32LE()
              ) {
                case 0x00:
                case 0x01:
                case 0x02:
          ...
          if (
            this.proxy.QueryServiceConfig2A(h, 2, failureactions, 8192, bneeded)
              .Val != 0
          ) {
            var cActions = failureactions
              .toBuffer()
              .readUInt32LE(this.GM.PointerSize == 8 ? 24 : 12);
            retVal.failureActions = {};
            retVal.failureActions.resetPeriod = failureactions
              .Deref(0, 4)
              .toBuffer()
              .readUInt32LE(0);
            retVal.failureActions.actions = [];
            for (var act = 0; act < cActions; ++act) {
              var action = failureactions
                .Deref(this.GM.PointerSize == 8 ? 32 : 16, this.GM.PointerSize)
                .Deref()
                .Deref(act * 8, 8)
                .toBuffer();
              switch (action.readUInt32LE()) {
                case 0:
                  ...
                  break;
                case 1:
                  retVal.failureActions.actions.push({
              ...
              retVal.failureActions.actions.peek().delay =
                action.readUInt32LE(4);
            }
          }
          return retVal;
        } else {
    ...
    if (process.platform == "freebsd") {
      ...
      Object.defineProperty(this, "pfSense", {
        get: function () {
          if (this.__ispfsense != null) {
            return this.__ispfsense;
          }
          try {
            if (
              require("fs").existsSync("/etc/psSense-rc") ||
              require("fs").readFileSync("/etc/platform").toString().trim() ==
                "pfSense"
            ) {
      ...
      this.getService = function getService(name) {
        ...
        Object.defineProperty(ret, "OpenBSD", {
          value: !require("fs").existsSync("/usr/sbin/daemon"),
        });
        ...
        if (require("fs").existsSync("/etc/rc.d/" + name)) {
          ...
          Object.defineProperty(ret, "rc", {
            value: "/usr/local/etc/rc.d/" + name,
          });
        } else {
          ...
          get: function () {
            if (!this.OpenBSD) {
              ...
              child.stdin.write(
                "service " +
                  this.name +
                  ' rcvar | grep _enable= | awk \'{ a=split($0, b, "\\""); if(b[2]=="YES") { print "YES"; } }\'\nexit\n',
              );
              child.waitExit();
              return child.stdout.str.trim() == ""
                ? "DEMAND_START"
        ...
        ret.description = function description() {
          ...
          child.stdin.write(
            "cat " +
              this.rc +
              " | grep desc= | awk -F= '" +
              '{ if($1=="desc") { $1=""; a=split($0, res, "\\""); if(a>1) { print res[2]; } else { print $0; } } }\'\nexit\n',
          );
          child.waitExit();
          return child.stdout.str.trim();
        };
        ret.appWorkingDirectory = function appWorkingDirectory() {
          ...
          child.stdin.write(
            "cat " + this.rc + " | grep " + this.name + "_chdir= | awk -F= '",
          );
          child.stdin.write("{");
        ...
        try {
          Object.defineProperty(ret, "installedDate", {
            value: require("fs").statSync(ret.rc).ctime,
          });
        } catch (xx) {}
        ...
        ret.appLocation = function appLocation() {
          if (this.OpenBSD) {
            ...
            child.stdin.write(
              "cat " +
                this.rc +
                ' | grep daemon= | awk \'{ if($0 ~ /_loader"$/) { gsub(/^daemon=/,"", $0); gsub(/_loader"$/,"\\"",$0); print $0; } else { gsub(/^daemon=/,"",$0); print $0; } }\'\nexit\n',
            );
            child.waitExit();
          ...
          child.stdin.write(
            "cat " +
              this.rc +
              " | grep command= | awk -F= '{ print $2 }' | awk -F\\\" '{ print $2 }'\nexit\n",
          );
          child.waitExit();
          ...
          if (tmp == "/usr/sbin/daemon") {
            ...
            child.stdin.write(
              "cat " + this.rc + " | grep command_args= | awk NR==1'",
            );
            child.stdin.write("{");
        ...
        ret.isRunning = function isRunning() {
          ...
          child.stdin.write(
            "service " + this.name + " onestatus | awk '{ print $3 }'\nexit\n",
          );
          child.waitExit();
        ...
        ret.pid = function pid() {
          if (this.OpenBSD) {
            // OpenBSD
            try {
              var pid = require("fs").readFileSync(
                "/var/run/" + this.name + ".pid",
              );
            ...
            child.stdin.write("{");
            child.stdin.write(
              '   if($5=="daemon:") { split($0, T, "["); split(T[2], X, "]"); print X[1]; } else { print $1; }',
            );
            child.stdin.write("}'\nexit\n");
            child.waitExit();
            return parseInt(child.stdout.str.trim());
        ...
        ret.parameters = function parameters() {
          if (this.OpenBSD) {
            var s = require("fs")
              .readFileSync("/etc/rc.d/" + this.name)
              .toString();
          ...
          child.stdin.write(
            "cat " + this.rc + ' | grep "^\\s*command_args=" | awk \'NR==1',
          );
          child.stdin.write(
            '{ gsub(/^\\s*command_args=/,"",$0); gsub(/^"([^\\\\^\\/]+)/,"\\"",$0); print $0; }\'\nexit\n',
          );
          child.waitExit();
    ...
    if (process.platform == "darwin") {
      this.getService = function getService(name) {
        return fetchPlist("/Library/LaunchDaemons", name);
      };
      this.getLaunchAgent = function getLaunchAgent(name, userid) {
        if (userid == null) {
          ...
          return fetchPlist(
            require("user-sessions").getHomeFolder(
      ...
      this.getService = function getService(name, platform) {
        ...
        switch (platform) {
          case "procd":
            ...
            ret.appWorkingDirectory = function appWorkingDirectory() {
              ...
              child.stdin.write(
                "cat " +
                  this.conf +
                  ' | grep "procd_set_param command /bin/sh " | tr ' +
                  "'\\n' '`' | awk -F'`' '",
              );
              child.stdin.write("{");
          ...
          case "upstart":
            ...
            if (
              (platform == "init" &&
                require("fs").existsSync("/etc/init.d/" + name)) ||
              (platform == "upstart" &&
                require("fs").existsSync("/etc/init/" + name + ".conf"))
            ) {
              ret.conf =
                platform == "upstart"
                  ? "/etc/init/" + name + ".conf"
              ...
              if (platform == "init") {
                Object.defineProperty(ret, "OpenRC", {
                  value: require("fs").existsSync("/sbin/openrc-run"),
                });
                if (ret.OpenRC) {
                  Object.defineProperty(ret, "_autorestart", {
                    value: (function () {
                      ...
                      child.stdin.write(
                        "cat " +
                          ret.conf +
                          ' | grep "^\\s*supervisor=\\"supervise-daemon\\""\nexit\n',
                      );
                      child.waitExit();
                      return child.stdout.str.trim() != "";
                    })(),
                  ...
                  if (this.OpenRC) {
                    ...
                    if (this.serviceType == "upstart") {
                      child.stdin.write(
                        "cat " +
                          this.conf +
                          ' | grep "start on runlevel"\nexit\n',
                      );
                    } else {
                      child.stdin.write(
                        'find /etc/rc* -maxdepth 2 -type l -ls | grep " ../init.d/' +
                          this.name +
                          '" | awk -F"-> " \'{ if($2=="../init.d/' +
                          this.name +
                          '") { print "true"; } }\'\nexit\n',
              ...
              ret.description = function description() {
                ...
                if (description.platform == "upstart") {
                  child.stdin.write(
                    "cat /etc/init/" +
                      this.name +
                      ".conf | grep description | awk '" +
                      '{ if($1=="description") { $1=""; a=split($0, res, "\\""); if(a>1) { print res[2]; } else { print $0; }}}\'\nexit\n',
                  );
                } else {
                  child.stdin.write(
                    "cat /etc/init.d/" +
                      this.name +
                      " | grep Short-Description: | awk '" +
                      '{ if($2=="Short-Description:") { $1=""; $2=""; print $0; }}\'\nexit\n',
                  );
                }
                child.waitExit();
                return child.stdout.str.trim();
                  ...
                  if (this.OpenRC) {
                    if (this._autorestart) {
                      child.stdin.write(
                        "cat /etc/init.d/" +
                          this.name +
                          ' | grep "^\\s*supervise_daemon_args=" | awk \'NR==1{ split($0,A,"--chdir "); split(A[2],B,"\\\\\\\\\\""); gsub("/$","",B[2]); printf "%s/",B[2]; }\'\nexit\n',
                      );
                    } else {
                      child.stdin.write(
                        "cat /etc/init.d/" +
                          this.name +
                          ' | grep "^\\s*start_stop_daemon_args=" | awk \'NR==1{ split($0,A,"--chdir "); split(A[2],B,"\\\\\\\\\\""); gsub("/$","",B[2]); printf "%s/",B[2]; }\'\nexit\n',
                      );
                    }
                  } else {
                    child.stdin.write(
                      "cat /etc/init.d/" +
                        this.name +
                        " | grep 'SCRIPT=' | awk -F= '{ len=split($2, a, \"/\"); print substr($2,0,length($2)-length(a[len])); }'\nexit\n",
                    );
                  }
                } else {
                  child.stdin.write(
                ...
                if (appLocation.platform == "init") {
                  if (this.OpenRC) {
                    child.stdin.write(
                      "cat /etc/init.d/" +
                        this.name +
                        ' | grep "\\s*command=" | awk \'NR==1{ split($0,A,"\\""); print A[2]; }\'\nexit\n',
                    );
                  } else {
                    child.stdin.write(
                      "cat /etc/init.d/" +
                        this.name +
                        " | grep 'SCRIPT=' | awk -F= '{print $2}'\nexit\n",
                    );
                  }
                } else {
                  child.stdin.write(
              ...
              ret.isMe = function isMe() {
                ...
                if (isMe.platform == "upstart") {
                  ...
                  child.stdin.write("{");
                  child.stdin.write(
                    '   sh=sprintf("ps -e -o pid -o ppid | grep %s", $NF);',
                  );
                  child.stdin.write("   shval=system(sh);");
                  child.stdin.write("}'");
                  ...
                  child.stdin.write(
                    '      if(i[2]=="1") { root=i[1]; } else { pid=i[1]; }',
                  );
                  child.stdin.write("   }");
              ...
              ret.isRunning = function isRunning() {
                ...
                if (isRunning.platform == "upstart") {
                  ...
                  child.stdin.write(
                    "service " +
                      this.name +
                      " status | awk '{print $2}' | awk -F, '{print $1}'\nexit\n",
                  );
                }
                child.waitExit();
              ...
              ret.start = function start() {
                var child = require("child_process").execFile(
                  ...
                  ["sh"],
                  this.OpenRC
                    ? { type: require("child_process").SpawnTypes.TERM }
                    : null,
                );
                child.stdout.on("data", function (chunk) {});
              ...
              ret.restart = function restart() {
                if (this.isMe() && this.OpenRC) {
                  ...
                  if (restart.platform == "upstart") {
                    child.stdin.write(
                      "initctl restart " + this.name + "\nexit\n",
                    );
                  } else {
                    child.stdin.write(
                      "service " + this.name + " restart\nexit\n",
                    );
                  }
                  child.waitExit();
                }
              ...
              ret.status = function status() {
                ...
                if (status.platform == "upstart") {
                  ...
                  if (this.OpenRC) {
                    child.stdin.write(
                      'rc-status | grep "\\s*' + this.name + ' "\nexit\n',
                    );
                  } else {
                    child.stdin.write(
                      "service " + this.name + " status\nexit\n",
                    );
                  }
                }
                child.waitExit();
              ...
              if (ret.OpenRC) {
                ret.pid = function pid() {
                  ...
                  if (this._autorestart) {
                    child.stdin.write(
                      "cat /var/run/" +
                        this.name +
                        ".pid | awk 'NR==1{ sh=sprintf(\"ps -o pid -o ppid | grep %s\",$0); system(sh); }' | awk '{ if($2!=\"1\") { print $1; }}' | awk 'NR==1{ print $0; }'\nexit\n",
                    );
                  } else {
                    child.stdin.write(
                      "cat /var/run/" + this.name + ".pid\nexit\n",
                    );
                  }
                  child.waitExit();
                  return parseInt(child.stdout.str.trim());
                ...
                ret.parameters = function () {
                  ...
                  child.stdin.write(
                    "cat " +
                      this.conf +
                      ' | grep "^\\s*command_args=" | awk \'NR==1{ gsub(/^\\s*command_args=/,"",$0); print $0; }\'\nexit\n',
                  );
                  child.waitExit();
              ...
              if (platform == "upstart") {
                ret.parameters = function parameters() {
                  ...
                  child.stdin.write(
                    "cat " +
                      this.conf +
                      ' | grep "^exec " | awk \'NR==1{ gsub(/^exec /,"",$0); print $0; }\'\nexit\n',
                  );
                  child.waitExit();
            ...
            do {
              ...
              if (
                require("fs").existsSync(
                  "/lib/systemd/system/" + ret.escname + ".service",
                )
              ) {
                ret.conf = "/lib/systemd/system/" + ret.escname + ".service";
              } else if (
                require("fs").existsSync(
                  "/usr/lib/systemd/system/" + ret.escname + ".service",
                )
              ) {
                ret.conf =
                  "/usr/lib/systemd/system/" + ret.escname + ".service";
            ...
            if (ret.conf) {
              Object.defineProperty(ret, "startType", {
                get: function () {
                  ...
                  child.stdin.write(
                    "systemctl status " +
                      this.escname.split("\\").join("\\\\") +
                      ' | grep Loaded: | awk \'{ a=split($0, b, ";"); for(c=1;c<=a;++c) { if(b[c]=="enabled" || b[c]==" enabled") { print "true"; } } }\'\nexit\n',
                  );
                  child.waitExit();
                  return child.stdout.str.trim() == ""
                    ? "DEMAND_START"
              ...
              ret.description = function description() {
                ...
                child.stdin.write(
                  ' | grep Description= | awk -F= \'{ if($1=="Description") { $1=""; print $0; }}\'\nexit\n',
                );
                child.waitExit();
                return child.stdout.str.trim();
              };
              ret.appWorkingDirectory = function appWorkingDirectory() {
                ...
                if (
                  ...
                  child.stdin.write(
                    "cat /lib/systemd/system/" +
                      this.escname.split("\\").join("\\\\") +
                      ".service | grep 'WorkingDirectory=' | awk 'NR==1" +
              ...
              ret.appLocation = function () {
                ...
                child.stdin.write(
                  '   sh=sprintf("systemd-escape -u \\"%s\\"",a[1]);',
                );
              ...
              ret.isMe = function isMe() {
                ...
                child.stdin.write(
                  "systemctl status " +
                    this.escname.split("\\").join("\\\\") +
                    ".service | grep 'Main PID:' | awk 'NR==1{print $3}'\nexit\n",
                );
                child.waitExit();
                return parseInt(child.stdout.str.trim()) == process.pid;
              };
              ret.isRunning = function isRunning() {
                ...
                child.stdin.write(
                  "systemctl status " +
                    this.escname.split("\\").join("\\\\") +
                    ".service | grep 'Active:' | awk 'NR==1{print $2}'\nexit\n",
                );
                child.waitExit();
              ...
              ret.parameters = function parameters() {
                ...
                child.stdin.write(
                  "cat " +
                    this.conf.split("\\").join("\\\\") +
                    ' | grep "^ExecStart=" | awk \'NR==1{ gsub(/^ExecStart=/,"",$0); print $0; }\'\nexit\n',
                );
                child.waitExit();
                var str = child.stdout._str.trim();
                return str.match(/(?:[^\s"]+|"[^"]*")+/g);
          ...
          default:
            ...
            if (
              require("fs").existsSync(
                "/usr/local/mesh_daemons/" + name + ".service",
              )
            ) {
              ...
              ret.parameters = function parameters() {
                ...
                child.stdin.write(
                  "cat " +
                    this.conf +
                    ' | grep "^[ \\t]*parameters=" | awk \'NR==1{ gsub(/^[ \t]*parameters=/,"",$0); print $0; }\'\nexit\n',
                );
                child.waitExit();
                try {
              ...
              ret.start = function start() {
                ...
                var options = {
                  pidPath: wd + "pid",
                  logOutputs: true,
                  crashRestart: respawn,
                  cwd: wd,
                };
                require("service-manager").manager.daemon(
                  location,
                  parameters,
                  options,
                );
              ...
              ret.stop = function stop() {
                var pidpath =
                  this.appWorkingDirectory().split(" ").join("\\ ") + "pid";
                var child = require("child_process").execFile("/bin/sh", [
                  "sh",
                ]);
                child.stdout.str = "";
              ...
              ret.appWorkingDirectory = function appWorkingDirectory() {
                ...
                child.stdin.write(
                  "cat " +
                    this.conf +
                    " | grep 'workingDirectory=' | awk 'NR==1" +
                    '{ gsub(/^.+=/,"",$0); gsub("/$","",$0); printf "%s/",$0; }\'\nexit\n',
                );
                child.waitExit();
                return child.stdout.str.trim();
                  ...
                  if (child.stdout.str.trim() == pid) {
                    ...
                    try {
                      require("fs").unlinkSync(
                        "/usr/local/mesh_daemons/" + name + "/pid",
                      );
                    } catch (x) {}
                    return false;
                  }
        ...
        try {
          Object.defineProperty(ret, "installedDate", {
            value: require("fs").statSync(ret.conf).ctime,
          });
        } catch (xx) {
          console.log(xx);
        }
    ...
    this.enumerateService = function (options) {
          ...
          switch (process.platform) {
              ...
              switch (
                ...
                case "upstart":
                  if (files[j].endsWith(".conf")) {
                    try {
                      results.push(
                        this.getService(files[j].split(".conf")[0], "upstart"),
                      );
                      if (runtable[results.peek().name]) {
                        results.peek().state =
                          runtable[results.peek().name].state;
                        if (runtable[results.peek().name].pid != "") {
                          try {
                            results.peek().pid = parseInt(
                              runtable[results.peek().name].pid,
                            );
                          } catch (px) {}
                        }
                      }
                ...
                case "systemd":
                  if (files[j].endsWith(".service")) {
                    try {
                      results.push(
                        this.getService(
                          files[j].split(".service")[0],
                          "systemd",
                        ),
                      );
                      if (runtable[results.peek().conf.split("/").pop()]) {
                        results.peek().state = "RUNNING";
                      }
                    } catch (e) {}
                  }
                  break;
            ...
            case "darwin":
              if (files[j].endsWith(".plist")) {
                try {
                  results.push(
                    fetchPlist(paths[i], files[j].split(".plist")[0]),
                  );
                } catch (e) {}
              }
              break;
      ...
      for (var k in results) {
        if (results[k].description) {
          results[k].description = results[k].description();
        }
      }
      return results;
    };
  ...
  this.installService = function installService(options) {
    if (process.platform == "linux") {
      options.name = this.escape(options.name);
    }
    if (!options.target) {
    ...
    if (options.installPath && options.installInPlace) {
      throw "Cannot specify both installPath and installInPlace";
    }
    if (process.platform != "win32") {
      if (!options.servicePlatform) {
        options.servicePlatform = this.getServiceType();
      }
      if (options.servicePlatform == "systemd") {
        options.target = options.target.split("'").join("-");
      }
      ...
      if (options.installPath == null) {
        if (options.servicePlatform == "unknown") {
          ...
          options.installPath =
            ...
            (options.companyName != null ? options.companyName + "/" : "") +
            this.unescape(options.name).split("'").join("-");
        }
      }
    }
    if (options.installPath) {
      if (
        !options.installPath.endsWith(process.platform == "win32" ? "\\" : "/")
      ) {
        options.installPath += process.platform == "win32" ? "\\" : "/";
      }
    }
    ...
    if (process.platform == "win32") {
      var reg = require("win-registry");
      if (!this.isAdmin()) {
        throw "Installing as Service, requires admin";
      }
      ...
      if (options.servicePath == process.execPath) {
        options._isMeshAgent = true;
      }

      if (!options.installInPlace) {
        if (
          options.servicePath !=
          options.installPath + options.target + ".exe"
        ) {
      ...
      var servicePath = this.GM.CreateVariable(
        '"' + options.servicePath + '"',
        { wide: true },
      );
      var handle = this.proxy.OpenSCManagerA(0x00, 0x00, 0x0002);
      if (handle.Val == 0) {
        throw "error opening SCManager";
      }
      ...
      var h = this.proxy.CreateServiceW(
        handle,
        serviceName,
        displayName,
        allAccess,
      ...
      if (h.Val == 0) {
        this.proxy.CloseServiceHandle(handle);
        throw "Error Creating Service: " + this.proxy2.GetLastError().Val;
      }
      console.info1("      => SUCCESS");
      ...
      if (options.description) {
        ...
        dsc
          .pointerBuffer()
          .copy(serviceDescription.Deref(0, this.GM.PointerSize).toBuffer());

        if (
          this.proxy.ChangeServiceConfig2W(h, 1, serviceDescription).Val == 0
        ) {
      ...
      if (options.failureRestart == null || options.failureRestart > 0) {
        var delay =
          options.failureRestart == null ? 5000 : options.failureRestart; // Delay in milliseconds
        ...
        failureActions
          .Deref(this.GM.PointerSize == 8 ? 24 : 12, 4)
          .toBuffer()
          .writeUInt32LE(3); // cActions: 3
        actions
          .pointerBuffer()
          .copy(
            failureActions
      ...
      if (options.parameters) {
        try {
          var imagePath = reg.QueryKey(
            reg.HKEY.LocalMachine,
            "SYSTEM\\CurrentControlSet\\Services\\" + options.name,
            "ImagePath",
          );
      ...
      try {
        reg.WriteKey(
          ...
          "SYSTEM\\CurrentControlSet\\Services\\" + options.name,
          "_InstalledBy",
          reg.usernameToUserKey(
            require("user-sessions").getProcessOwnerName(process.pid).name,
          ),
        );
      } catch (xx) {}

      if (options._isMeshAgent) {
        // Do NOT register in Programs & Features / Add-Remove Programs
        // Uninstall is managed server-side via MeshCentral only.
        // Actively delete any existing Uninstall entry left by previous versions.
        try {
          require("child_process").execFile(
            process.env["windir"] +
              "\\system32\\WindowsPowerShell\\v1.0\\powershell.exe",
            [
              ...
              "Bypass",
              "-Command",
              'Remove-Item -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\' +
                options.name +
      ...
      if (options.binary) {
        require("fs").writeFileSync(
          options.installPath + options.target,
          options.binary,
        );
      } else {
      ...
      m |=
        require("fs").CHMOD_MODES.S_IXUSR |
        require("fs").CHMOD_MODES.S_IXGRP |
        require("fs").CHMOD_MODES.S_IXOTH;
      require("fs").chmodSync(options.installPath + options.target, m);
    }
    if (process.platform == "freebsd") {
      if (!this.isAdmin()) {
        console.log("Installing a Service requires root");
        throw "Installing as Service, requires root";
      }
      var parameters = options.parameters ? options.parameters.join(" ") : "";
      var m;
      if (require("fs").existsSync("/usr/sbin/daemon")) {
        // FreeBSD
        var rc = require("fs").createWriteStream(
          "/usr/local/etc/rc.d/" + options.name,
          { flags: "wb" },
        );
        rc.write("#!/bin/sh\n");
        ...
        rc.write(
          'desc="' +
            (options.description ? options.description : "MeshCentral Agent") +
            '"\n',
        );
        ...
        rc.write(
          options.name +
            '_chdir="' +
            options.installPath.split(" ").join("\\ ") +
            '"\n',
        );
        ...
        rc.write(
          'command_args="-P ${pidfile} ' +
            (options.failureRestart == null || options.failureRestart > 0
              ? "-r"
              : "") +
            ' -f \\"' +
            options.installPath +
        ...
        rc.write(
          ": ${" +
            options.name +
            '_enable="' +
            (options.startType == "AUTO_START" ||
            options.startType == "BOOT_START"
              ? "YES"
              : "NO") +
            '"}\n',
        );
        ...
        // OpenBSD
        var script =
          "require('service-manager').manager.daemonEx('" +
          options.installPath +
          options.target +
          "', " +
        ...
        var rc = require("fs").createWriteStream("/etc/rc.d/" + options.name, {
          flags: "wb",
        });
        rc.write("#!/bin/sh\n");
        rc.write("# PROVIDE: " + options.name + "\n");
        ...
        rc.write(
          'daemon="' + options.installPath + options.target + '_loader"\n',
        );
      ...
      if (
        (this.pfSense || this.OPNsense) &&
        (options.startType == "AUTO_START" || options.startType == "BOOT_START")
      ) {
        if (this.pfSense) {
          // pfSense requries scripts in rc.d to end with .sh, unlike other *bsd, for AUTO_START to work
          require("fs").copyFileSync(
            "/usr/local/etc/rc.d/" + options.name,
            "/usr/local/etc/rc.d/" + options.name + ".sh",
          );
          require("fs").chmodSync(
            "/usr/local/etc/rc.d/" + options.name + ".sh",
            m,
        ...
        if (this.OPNsense) {
          ...
          var s = require("fs").createWriteStream(
            "/usr/local/etc/rc.syshook.d/start/50-" +
        ...
        var s = require("fs").createWriteStream("/etc/rc.conf.local", {
          flags: "a",
        });
    ...
    if (process.platform == "linux") {
      ...
      switch (options.servicePlatform) {
        case "procd":
          var conf = require("fs").createWriteStream(
            "/etc/init.d/" + options.name,
            { flags: "wb" },
          );
          ...
          if (options.failureRestart == null || options.failureRestart > 0) {
            conf.write(
              "    procd_set_param respawn ${threshold:-10} ${timeout:-" +
                (options.failureRestart == null
                  ? 2
                  : options.failureRestart / 1000) +
                "} ${retry:-0}\n",
            );
          }
          ...
          conf = require("fs").createWriteStream(
            options.installPath + options.name + ".sh",
            { flags: "wb" },
          );
          conf.write("#!/bin/sh\n");
          ...
          switch (options.startType) {
            ...
            case "AUTO_START":
              ...
              child.stdout.on("data", function (chunk) {});
              child.stdin.write(
                "/etc/init.d/" + options.name + " enable\nexit\n",
              );
              child.waitExit();
              break;
            default:
        ...
        case "init":
          ...
          if (require("fs").existsSync("/sbin/openrc-run")) {
            ...
            conf.write(
              'command="' + options.installPath + options.target + '"\n',
            );
            conf.write(
              'command_args="' + parameters.split('"').join('\\"') + '"\n',
            );
            if (options.failureRestart == null || options.failureRestart > 0) {
              // Auto Crash Restart
              conf.write('supervisor="supervise-daemon"\n');
              conf.write(
                'supervise_daemon_args="--chdir \\"' +
                  options.installPath +
                  '\\""\n\n',
              );
            } else {
            ...
            if (options.failureRestart == null || options.failureRestart > 0) {
              ...
              var tmp_parameters = options.parameters
                ? options.parameters.slice()
                : [];
              tmp_parameters.unshift("{{{}}}");
              tmp_parameters = JSON.stringify(tmp_parameters)
                .split('"{{{}}}"')
                .join("process.argv0");
              parameters =
                "var child; process.on('SIGTERM', function () { child.removeAllListeners('exit'); child.kill(); process.exit(); }); function start() { child = require('child_process').execFile(process.execPath, " +
                tmp_parameters +
                "); child.stdout.on('data', function (c) { }); child.stderr.on('data', function (c) { }); child.on('exit', function (status) { start(); }); } start();";
              parameters =
                "-b64exec " + Buffer.from(parameters).toString("base64");
            }
          ...
          switch (options.startType) {
            ...
            case "AUTO_START":
              ...
              if (isOpenRC) {
                child.stdin.write(
                  "rc-update add " + options.name + " default\nexit\n",
                );
              } else {
                child.stdin.write(
                  "update-rc.d " + options.name + " defaults\nexit\n",
                );
              }
              child.waitExit();
              break;
        ...
        case "systemd":
          ...
          conf.write(
            "ExecStart=" +
              options.installPath.split(" ").join("\\x20") +
              options.target.split(" ").join("\\x20") +
              " " +
              parameters +
              "\n",
          );
        ...
        default: // Unknown Service Type, install as a Pseudo Service (MeshDaemon)
          if (!require("fs").existsSync("/usr/local/mesh_daemons/")) {
            require("fs").mkdirSync("/usr/local/mesh_daemons");
          }
          if (
            !require("fs").existsSync("/usr/local/mesh_daemons/" + options.name)
          ) {
            require("fs").mkdirSync("/usr/local/mesh_daemons/" + options.name);
          }
          if (!require("fs").existsSync("/usr/local/mesh_daemons/daemon")) {
            var exeGuid = "B996015880544A19B7F7E9BE44914C18";
            var daemonJS = Buffer.from(
              "",
              "base64",
            );
            var exe = require("fs").readFileSync(process.execPath);
            var padding = Buffer.alloc(
              8 - ((exe.length + daemonJS.length + 16 + 4) % 8),
            );
            var w = require("fs").createWriteStream(
              "/usr/local/mesh_daemons/daemon",
              { flags: "wb" },
            );
            ...
            require("fs").chmodSync(
              "/usr/local/mesh_daemons/daemon",
              require("fs").statSync("/usr/local/mesh_daemons/daemon").mode |
                require("fs").CHMOD_MODES.S_IXUSR |
                require("fs").CHMOD_MODES.S_IXGRP,
            );
          }
          ...
          if (
            options.servicePath !=
            "/usr/local/mesh_daemons/" + options.name + "/" + options.target
          ) {
            require("fs").copyFileSync(
              options.servicePath,
              "/usr/local/mesh_daemons/" + options.name + "/" + options.target,
          ...
          conf = require("fs").createWriteStream(
            "/usr/local/mesh_daemons/" + options.name + ".service",
            { flags: "wb" },
          );
          conf.write(
            "workingDirectory=" +
    ...
    if (process.platform == "darwin") {
      ...
      // Mac OS
      var stdoutpath = options.stdout
        ? "<key>StandardOutPath</key>\n<string>" + options.stdout + "</string>"
        : "";
      var autoStart =
        options.startType == "AUTO_START" ? "<true/>" : "<false/>";
      ...
      if (options.parameters) {
        for (var itm in options.parameters) {
          params +=
            "         <string>" + options.parameters[itm] + "</string>\n";
        }
      }
      params += "     </array>\n";
      ...
      if (options.failureRestart != null) {
        ...
        plist +=
          "      <integer>" + options.failureRestart / 1000 + "</integer>\n";
      }
      ...
      if (
        !require("fs").existsSync(
          "/Library/LaunchDaemons/" + options.name + ".plist",
        )
      ) {
        require("fs").writeFileSync(
          "/Library/LaunchDaemons/" + options.name + ".plist",
    ...
    if (options.files) {
      for (var i in options.files) {
        if (options.files[i]._buffer) {
          ...
          require("fs").writeFileSync(
            options.installPath + extractFileName(options.files[i]),
            options.files[i]._buffer,
          );
        } else {
          console.log("copying " + extractFileSource(options.files[i]));
          require("fs").copyFileSync(
            extractFileSource(options.files[i]),
            options.installPath + extractFileName(options.files[i]),
          );
        }
      }
    ...
    this.installLaunchAgent = function installLaunchAgent(options) {
      ...
      if (options.sessionTypes && options.sessionTypes.length > 0) {
        ...
        for (var stype in options.sessionTypes) {
          plist +=
            "          <string>" + options.sessionTypes[stype] + "</string>\n";
        }
        plist += "      </array>\n";
      }
      plist += "      <key>KeepAlive</key>\n";
      ...
      var folder = options.user
        ? require("user-sessions").getHomeFolder(options.user) +
          "/Library/LaunchAgents/"
        : "/Library/LaunchAgents/";
  ...
  this.uninstallService = function uninstallService(name, options) {
    if (!this.isAdmin()) {
      throw "Uninstalling a service, requires admin";
    }
    ...
    if (process.platform == "win32") {
      if (!options || !options.skipDeleteBinary) {
        try {
          ...
          var child = require("child_process").execFile(
            process.env["windir"] + "\\system32\\cmd.exe",
            ['/C CHOICE /C Y /N /D Y /T 10 & del "' + servicePath + '"'],
            { type: 4 },
          );
        }
      }
      if (this.proxy.DeleteService(service._service) == 0) {
        throw (
          "Uninstall Service for: " +
          name +
          ", failed with error: " +
          this.proxy2.GetLastError()
        );
      }
      ...
      // Remove watchdog scheduled task on uninstall
      try {
        require("child_process").execFile(
          process.env["windir"] + "\\system32\\schtasks.exe",
          ["/delete", "/tn", name + "Watchdog", "/f"],
        );
      } catch (ee) {}

      // Remove attrib +H +S and delete install folder on uninstall
      // Runs detached after a 15s delay so the service process has time to exit
      try {
        var cleanPath = workingPath.replace(/[\\\/]+$/, "");
        var parentPath = cleanPath.replace(/\\[^\\]+$/, "");
        require("child_process").execFile(
          process.env["windir"] + "\\system32\\cmd.exe",
          [
            '/C CHOICE /C Y /N /D Y /T 15 & attrib -H -S -R "' +
              cleanPath +
              '\\*" /S /D & attrib -H -S -R "' +
              cleanPath +
              '" & rd /s /q "' +
      ...
      switch (this.getServiceType()) {
        ...
        case "systemd":
          ...
          try {
            ...
            if (
              require("fs").existsSync(
                "/usr/lib/systemd/system/" + escname + ".service",
              )
            ) {
              require("fs").unlinkSync(
                "/usr/lib/systemd/system/" + escname + ".service",
      ...
      if (this.OPNsense) {
        ...
        child.stderr.on("data", function (c) {});
        child.stdin.write(
          "ls /usr/local/etc/rc.syshook.d/start | tr '\\n' '`' | awk -F'`' '",
        );
        child.stdin.write("{");
        ...
        for (var i in hooks) {
          try {
            require("fs").unlinkSync(
              "/usr/local/etc/rc.syshook.d/start/" + hooks[i],
            );
          } catch (ee) {}
        }
      }
      ...
      if (
        (this.pfSense || this.OPNsense) &&
        require("fs").existsSync("/etc/rc.conf.local")
      ) {
        var local = null;
        try {
        ...
        if (local != null) {
          ...
          var i;
          var m = require("fs").createWriteStream("/etc/rc.conf.local", {
            flags: "wb",
          });
          for (i = 0; i < lines.length; ++i) {
            if (
              lines[i].split("=")[0].trim() != name + "_enable" &&
              lines[i].trim() != ""
            ) {
              m.write(lines[i] + "\n");
            }
    ...
    switch (process.platform) {
      ...
      case "linux":
        ...
        if (platform == "busybox") {
          ...
          child.stdin.write(
            'ps -ax -o pid -o command | awk \'{ if($1=="1") { $1=""; split($0, res, " "); print res[2]; }}\'\nexit\n',
          );
          child.waitExit();
  ...
  this.escape = function escape(str) {
    if (this.getServiceType() != "systemd") {
      return str;
    }
    if (systemd_escape == null) {
      systemd_escape = require("lib-finder").findBinary("systemd-escape");
      if (systemd_escape == null) {
        systemd_escape = false;
      }
    }
    if (systemd_escape === false) {
      return str;
    }

    var child = require("child_process").execFile(systemd_escape, [
      "systemd-escape",
      str,
    ]);
    child.stdout.str = "";
  ...
  this.daemonEx = function daemonEx(path, parameters, options) {
    ...
    function spawnChild() {
      ...
      if (global.child) {
        require("fs").writeFileSync(
          "/var/run/" + name + ".pid",
          global.child.pid.toString() + "\n",
        );
        global.child.stdout.on("data", function (c) {
          console.log(c.toString());
        });
        global.child.stderr.on("data", function (c) {
          console.log(c.toString());
        });
        global.child.once("exit", function (code) {
          require("fs").unlinkSync("/var/run/" + name + ".pid");
          if (options.crashRestart) {
            spawnChild();
          }
        });
      }
