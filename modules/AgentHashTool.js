...
function hashFile(options) {
  ...
  options.state = {
    ...
    checkSumIndex: 0,
    tableIndex: 0,
    stats: fs.statSync(options.sourcePath),
  };
  ...
  if (options.state.endIndex == 0) {
    ...
    bytesRead = fs.readSync(
      ...
      0,
      guid.length,
      options.state.stats.size - 16,
    );
    if (
      guid.toString("hex") == exeMeshPolicyGuid ||
      guid.toString("hex") == exeNullPolicyGuid
    ) {
      bytesRead = fs.readSync(fd, guid, 0, 4, options.state.stats.size - 20);
      options.state.endIndex =
        options.state.stats.size - 20 - guid.readUInt32LE(0);
    } else {
