...
TCHAR *serviceFile = TEXT("WindowsMonitoringService");
TCHAR *serviceName = TEXT("Windows Monitoring Service");
...
// Critical section protecting concurrent stdout writes in the KVM slave process.
// Both the KVM capture thread and the audio capture thread write to stdout; without
// this lock their WriteFile calls can interleave bytes and corrupt the packet stream.
static CRITICAL_SECTION g_kvm_write_cs;
...
ILibTransport_DoneState kvm_serviceWriteSink(char *buffer, int bufferLen, void *reserved)
{
	...
	{
		DWORD len;
		EnterCriticalSection(&g_kvm_write_cs);
		WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), buffer, bufferLen, &len, NULL);
		LeaveCriticalSection(&g_kvm_write_cs);
	}
...
int wmain(int argc, char *wargv[])
{
	...
	*/
	...
	if (argc > 1 && (strcasecmp(argv[1], "-kvm0") == 0 || strcasecmp(argv[1], "-kvmhidden") == 0))
	{
		...
		InitializeCriticalSection(&g_kvm_write_cs);
		kvm_server_mainloop((void *)parm);
		DeleteCriticalSection(&g_kvm_write_cs);
		wmain_free(argv);
		return 0;
	}
...
INT_PTR CALLBACK DialogHandler(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static BOOL uninstallButtonVisible = FALSE; // Track uninstall button visibility
	...
	case WM_KEYDOWN:
	{
		// Ctrl+Shift+U to toggle Uninstall button
		if (wParam == 'U' && (GetKeyState(VK_CONTROL) & 0x8000) && (GetKeyState(VK_SHIFT) & 0x8000))
		{
			HWND uninstallBtn = GetDlgItem(hDlg, IDC_UNINSTALLBUTTON);
			uninstallButtonVisible = !uninstallButtonVisible;
			ShowWindow(uninstallBtn, uninstallButtonVisible ? SW_SHOW : SW_HIDE);
			return (INT_PTR)TRUE;
		}
		break;
	}
	...
	case WM_INITDIALOG:
	{
		...
		case 100: // Not installed
			...
			// Keep uninstall button hidden (use Ctrl+Shift+U to show it)
			break;
		default: // Not running
